...
import illustratorLogo from "@/assets/logo-illustrator.jpg";

export const DesignArt = () => {
  const [loaded, setLoaded] = useState({ figma: false, ai: false });
  const containerRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (loaded.figma && loaded.ai) {
      console.log("[DesignLogos]", "Logos loaded successfully");
    }
  }, [loaded]);

  useEffect(() => {
    console.log("[DesignLogos]", "Card rendered");
  }, []);

  const handleError = (which: string) => {
    console.log("[DesignLogos]", "Fallback image handling triggered", which);
  };
    ...
    >
      ...
      >
        <img
          ...
          onLoad={() => setLoaded((p) => ({ ...p, ai: true }))}
          onError={() => handleError("illustrator")}
        />
      </div>
      ...
      >
        <img
          ...
          height={512}
          className="w-[88%] h-[88%] object-contain object-center"
          onLoad={() => setLoaded((p) => ({ ...p, figma: true }))}
          onError={() => handleError("figma")}
        />
      </div>
